...
public class UIFader : MonoBehaviour
{
    ...
    public float automaticFadeInDelay = 1;
    ...
    private void Awake()
    {
        if (automaticFadeIn)
        {
            ...
            LeanTween.delayedCall(automaticFadeInDelay, FadeIn);
        }        
    ...
    private void Initialise()
    {
        ...
        childImages = Array.FindAll(childImages, image => image.GetComponent<UIFader>() == null || image.gameObject == this.gameObject);   
        ...
        childTextElements = Array.FindAll(childTextElements, text => text.GetComponent<UIFader>() == null || text.gameObject == this.gameObject);   
    ...
    public void Show(float duration = -1, Action action = null)
    {
        if (duration < 0)
        {
        ...
        LeanTween.value(gameObject, 0, 1, duration).setOnComplete(action);
    ...
    public void Dismiss(float duration = -1, Action action = null)
    {
        if (duration < 0)
        {
            duration = transitionDuration;
